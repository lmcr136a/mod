...
origin_model.load_state_dict(torch.load(args.honey_model, map_location=device))
...
def load_resnet_honey_model(model, random_rule):
    ...
    print(list(state_dict.values())[6].shape)
...
def calculationFitness(honey, train_loader, args):
    ...
    if args.arch == 'vgg_cifar':
        ...
        print(list(model.state_dict().values())[6].shape)
...
def main():
    ...
    if args.resume == None:
        ...
        if args.best_honey == None:
            ...
            for i, h in enumerate(best_honey.code):
                best_honey.code[i] = int(h[0])
        ...
        for i, h in enumerate(args.lr_decay_step):
            chrs = ""
            for string in h:
                chrs += string
            args.lr_decay_step[i] = int(chrs)
        print(args.lr_decay_step)
