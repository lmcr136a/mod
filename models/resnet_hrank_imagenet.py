...
class ResBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, cp_rate=[0.], tmp_name=None):
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes, num_layers, covcfg=None, compress_rate=None):
        self.inplanes = 64
    ...
    def _make_layer(self, block, planes, blocks, stride, cp_rate, tmp_name):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride=stride, cp_rate=cp_rate[1:4],
...
def resnet50_hrank(num_classes, compress_rate=None):
    cov_cfg = [(3*i + 3) for i in range(3*3 + 1 + 4*3 + 1 + 6*3 + 1 + 3*3 + 1 + 1)]
    model = ResNet(ResBottleneck, [3, 4, 6, 3],num_classes, num_layers=50, covcfg=cov_cfg, compress_rate=compress_rate)
    return model

def resnet34_hrank(num_classes, compress_rate=None):
    cov_cfg = [(3*i + 2) for i in range(3*2 + 1 + 4*2 + 1 + 6*2 + 1 + 3*2 + 1 + 1)]
    model = ResNet(ResBasicBlock, [3, 4, 6, 3],num_classes, num_layers=34, covcfg=cov_cfg, compress_rate=compress_rate)
    return model