...
class VGG(nn.Module):
    def __init__(self, num_classes=10, init_weights=True, is_sparse=False, cfg=None, index=None):
        ...
        if is_sparse:
            self.features, in_channels = self.make_sparse_layers(cfg[:-1], True)
            ...
            self.features, in_channels = self.make_layers(cfg[:-1], True)
            self.classifier = nn.Sequential(OrderedDict([
                ('linear1', nn.Linear(in_channels, cfg[-1])),
                ('norm1', nn.BatchNorm1d(cfg[-1])),
                ('relu1', nn.ReLU(inplace=True)),
    ...
    def make_sparse_layers(self, cfg, batch_norm=True):
        ...
        return sparse_layers, in_channels
...
def vgg_16_bn_gal(**kwargs):
    model = VGG(**kwargs)
    return model

def vgg_16_bn_sparse(num_classes, **kwargs):
    model = VGG(is_sparse=True, num_classes=num_classes, **kwargs)
    return model
