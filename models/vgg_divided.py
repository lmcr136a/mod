...
class VGG(nn.Module):
    def __init__(self, sparsity, target_classes, cfg=None, num_classes=10):
        ...
        self.features1, self.features2, self.features3, self.features4, in_channels = self._make_layers(cfg)
    ...
    def _make_layers(self, cfg):
        ...
        inter2 = 7
        inter3 = 11
        for i, x in enumerate(cfg[:inter1]):
            if x == 'M':
        ...
        for i, x in enumerate(cfg[inter1:inter2]):
            i += inter1
            if x == 'M':
        ...
        for i, x in enumerate(cfg[inter2:inter3]):
            i += inter1
            if x == 'M':
                layers3.add_module('pool%d' % i, nn.MaxPool2d(kernel_size=2, stride=2))
            else:
                x = int(x * (1-self.sparsity[cnt]))
                ...
                # print(in_channels, x)
                conv2d = nn.Conv2d(in_channels, x, kernel_size=3, padding=1)
                layers3.add_module('conv%d' % i, conv2d)
                layers3.add_module('norm%d' % i, nn.BatchNorm2d(x))
                layers3.add_module('relu%d' % i, nn.ReLU(inplace=True))
                in_channels = x
        
        for i, x in enumerate(cfg[inter3:]):
            i += inter1
            if x == 'M':
                layers4.add_module('pool%d' % i, nn.MaxPool2d(kernel_size=2, stride=2))
            else:
                x = int(x * (1-self.sparsity[cnt]))
                ...
                # print(in_channels, x)
                conv2d = nn.Conv2d(in_channels, x, kernel_size=3, padding=1)
                layers4.add_module('conv%d' % i, conv2d)
                layers4.add_module('norm%d' % i, nn.BatchNorm2d(x))
                layers4.add_module('relu%d' % i, nn.ReLU(inplace=True))
                in_channels = x
        return layers1, layers2, layers3, layers4, in_channels
