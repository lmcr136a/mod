...
filenames = ["vgg16_bn_cifar100_f1234"]
...
for filename in filenames:
    ...
    for key in keys:
        if "features1" in key:
            if int(key.split(".")[1][4:]) < 5:
                new_key = key.replace("features1", "features1")
                # pass
            else:
                new_key = key.replace("features1", "features2")
            # new_key = key.replace("features1", "features1")
            sd[new_key] = sd[key]
            del sd[key]
            keys.remove(key)
    ...
    # print(sd.keys())
    torch.save(sd, f"models/pretrained/vgg16_bn_cifar100_f1234.pt")